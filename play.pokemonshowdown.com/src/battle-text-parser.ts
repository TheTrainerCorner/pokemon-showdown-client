...
class BattleTextParser {
	...
	static upgradeArgs({args, kwArgs}: {args: Args, kwArgs: KWArgs}): {args: Args, kwArgs: KWArgs} {
		...
		case '-activate': {
			...
			if ([
				...
				'electricterrain', 'mistyterrain', 'psychicterrain', 'myriadterrain', 'telepathy', 'stickyhold', 'suctioncups', 'aromaveil',
	...
	parseArgsInner(args: Args, kwArgs: KWArgs) {
		...
		case '-fail': {
			...
			templateId = 'fail';
			if (['brn', 'frb', 'frz', 'par', 'psn', 'slp', 'substitute', 'shedtail', 'phc'].includes(id)) {
